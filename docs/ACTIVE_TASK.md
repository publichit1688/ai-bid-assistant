...
P4-02第五小批：提升正式 `1.0.0` 并创建本地发布标签。

状态：已完成。经项目负责人明确授权，版本已统一提升为 `1.0.0`，本提交用于本地 `v1.0.0` 发布标签；不执行远程推送或真实服务器部署。
...
P4本地功能冻结完成。下一批进入P5-01，仅设计V1.5目录生成与人工编辑的数据模型及交互原型；远程推送和真实服务器部署如需执行，仍须单独明确授权。

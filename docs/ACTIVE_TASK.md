...
P5-06第二小批：建立 `1.5.0-rc.1` 本地候选基线。

状态：已完成。经用户授权统一根版本、FastAPI读取版本及前端包/锁文件为 `1.5.0-rc.1`，复跑完整候选门禁并建立单一本地原子候选提交；未创建标签、推送或部署。
...
进入P5-07第一小批：只读复核 `1.5.0-rc.1` 候选提交、提交内容和验收清单，形成负责人签署材料；未经授权不提升正式版本、不创建标签、不推送。

...
P6-12：将已签署候选提升为正式 `1.5.1`。

状态：已完成。经项目负责人授权，根版本、FastAPI及前端包/锁文件统一为正式 `1.5.1`，完整正式发布门禁通过并创建本地发布提交。未创建标签、推送或部署。
...
P6-13等待项目负责人决定是否为正式 `1.5.1` 创建本地 `v1.5.1` 标签。未经另行明确授权，不创建标签、不推送、不部署。

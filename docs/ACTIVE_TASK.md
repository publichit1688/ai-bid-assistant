...
P5-08：建立 `1.5.0-rc.1` 签署记录本地文档提交。

状态：已完成。经项目负责人授权，最终候选提交 `33ad2bc` 的签署单和状态同步已建立为独立本地文档提交；提交哈希以本批Git结果为准。不创建标签、不推送、不提升正式版本、不部署。
...
签署记录提交完成后停止，等待项目负责人后续决定是否启动正式 `1.5.0` 提升流程；未经明确授权不修改版本、不创建标签、不推送、不部署。

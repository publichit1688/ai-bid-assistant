...
P6-11：创建包含 `1.5.1-rc.1` 签署记录的独立本地文档提交。

状态：已完成。经项目负责人授权，验收签署单、候选审计和项目状态作为独立本地文档提交落库；候选提交 `814c4bf` 保持在父提交历史中。未提升正式版本、创建标签、推送或部署。
...
P6-12等待项目负责人决定是否将已签署候选提升为正式 `1.5.1`。未经另行明确授权，不修改版本、不创建标签、不推送、不部署。

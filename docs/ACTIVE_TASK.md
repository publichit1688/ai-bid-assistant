...
P4-02第一小批：建立 `1.0.0-rc.1` 本地Git候选基线。

状态：已完成。本提交建立 `1.0.0-rc.1` 本地原子候选基线；本次不推送、不打标签，也不提升为正式 `1.0.0`。
...
候选基线提交并复核成功后，停止本批；后续是否创建正式 `v1.0.0` 标签、签署发布清单或推送远程，必须另行获得用户明确授权。

...
P4-02第四小批：记录 `1.0.0-rc.1` 候选验收签署。

状态：已完成。项目负责人已确认最终候选提交 `6844019` 的验收，本提交仅记录签署日期、范围和限制；不推送、不打标签，也不提升为正式 `1.0.0`。
...
等待项目负责人决定是否将已签署候选提升为正式 `1.0.0`。版本提升、`v1.0.0` 标签和远程推送均需分别明确授权；在此之前保持候选状态。

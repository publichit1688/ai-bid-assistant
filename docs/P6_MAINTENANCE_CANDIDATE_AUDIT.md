...
签署前只读复核已确认候选身份、22个文件、版本、标签和数据排除边界；项目负责人已签署候选提交 `814c4bf`，并另行授权提升为本地正式 `1.5.1`。验收记录见 `docs/V1_5_1_ACCEPTANCE_SIGNOFF.md`，发布说明见 `docs/V1_5_1_RELEASE_NOTES.md`。标签、推送和部署仍需另行明确授权；任何回滚使用可审计的 `git revert`，不使用 `git reset --hard`，也不删除数据库、上传文件或报告。
